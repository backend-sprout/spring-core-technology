...
위 같은 요소들을 고려해보면, 핵심 기능 로직에서 부가 기능 로직을 제거해야한다.     
...
`cross-cutting concern`에서 알 수 있듯이 각각의 영역에서 중복된 코드가 발생하고 있다.           
그리고 이들은 각각의 영역에서 별개로 코드를 작성하고 관리하기에 유지보수에 어려움이 발생한다.        
