...
# 환경 프로퍼티 파일   
`Spring boot` 프로퍼티 파일은 **설정 관련 및 기타 정적인 값을 `키/값` 형식으로 관리한다.**           
...
최근에는 표현의 한계로 **YAML 파일을 더 많이 사용합니다.**         
...
yaml을 설정하려면 `SnakeYAML 라이브러리`를 포함해야 하지만 `springboot-starter`에 기본적으로 내장되어있다.             
